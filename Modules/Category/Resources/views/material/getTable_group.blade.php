<div class="table-responsive">
    <table id="table_" class="table table-striped table-bordered table-responsive" style="width:100%">
        {{-- {{dd($datas)}} --}}
            <thead>
                <tr>
                    <th style="min-width:50px">รหัส</th>
                    <th style="min-width:100px">กลุ่ม</th>
                   <th style="min-width:150px">รายละเอียด </th> 
                    <th style="min-width:120px">หน่วยนับ </th>
                    <th style="min-width:100px">ราคา</th>
                    <th align="center" style="min-width:10px">View</th>
                    @if (Session::get('auth')['role_id'] ==='1')  <th style="min-width:80px">Tool</th> @endif
                </tr>
            </thead>
            <tbody>
    ...
    <script>
            ...
            function delete_(group_id) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{url('category/delete_material_group')}}",
                    data: {
                        _token : '{{ csrf_token()}}',
                        group_id : group_id,
                    },
            ...
            function view(group_id) {
                ...
                $.ajax({
                    type: "get",
                    url: "{{url('category/view_material_group')}}",
                    data: {
                        _token : '{{csrf_token()}}',
                        group_id : group_id,
                    },
