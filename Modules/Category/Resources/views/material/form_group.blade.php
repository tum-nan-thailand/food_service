      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">@if(isset($datas))
                {{'แก้ไขกลุ่มวัตถุดิบ'}}
                @else{{'เพิ่มกลุ่มวัตถุดิบ'}} @endif</h4>
        </div>
        <div class="modal-body">
                ...
                {{url('category/material_group_update')}}
                @else {{url('category/material_group_save')}} @endif">
                        @csrf
                        <div class="col-xs-6">
                            ...
                            <div class="col-xs-6">
                                ...
                                <input type="text" class="form-control "   required  value="@isset($datas){{$datas->unit}}@endisset" name="unit"  id="" placeholder="หน่วยนับ">
                                </div>
                            </div>
                            ...
                            <div class="col-xs-6">
                                ...
                                <input type="number" class="form-control "  required  value="@isset($datas){{$datas->price}}@endisset" name="price"  id="" placeholder="ราคา">
                                </div>
                            </div>
...
<script>
        ...
        $('#group_id').blur(function (e) {
            e.preventDefault();
                            $.ajax({
                                    type: "get",
                                    url: "{{url('category/check_groupid_duplicate')}}",
                                    data: {
                                        _token : '{{csrf_token()}}',
                                        group_id : $(this).val(),
                                    },
